...
static void addPGOAndCoverageFlags(const ToolChain &TC, Compilation &C,
  ...
  if (Args.hasArg(options::OPT_fdrop_align_attr_exclude_func_EQ)) {
    auto *Arg = Args.getLastArg(options::OPT_fdrop_align_attr_exclude_func_EQ);
    StringRef v = Arg->getValue();
    CmdArgs.push_back(Args.MakeArgString(Twine("-fdrop-align-attr-exclude-func=" + v)));
  }

  if (Args.hasArg(options::OPT_fdrop_align_attr_exclude_func_args_EQ)) {
    auto *Arg = Args.getLastArg(options::OPT_fdrop_align_attr_exclude_func_args_EQ);
    StringRef v = Arg->getValue();
    CmdArgs.push_back(Args.MakeArgString(Twine("-fdrop-align-attr-exclude-func-args=" + v)));
  }
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Args.hasFlag(options::OPT_fdrop_align_attr,
                   options::OPT_fno_drop_align_attr, false))
