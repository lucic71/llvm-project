...
void CodeGenModule::ConstructAttributeList(StringRef Name,
  ...
  if (TargetDecl) {
    ...
    if (const FunctionDecl *Fn = dyn_cast<FunctionDecl>(TargetDecl)) {
      ...
      if (AttrOnCallSite && Fn->isReplaceableGlobalAllocationFunction()) {
        ...
        if (getCodeGenOpts().AssumeSaneOperatorNew &&
          ...
          RetAttrs.addAttribute(llvm::Attribute::NoAlias); // no need to guard with -fdrop-noalias-restrict-attr
      }
    ...
    if (EnableIgnorePureConstAttrs && TargetDecl->hasAttr<ConstAttr>()) {
      FuncAttrs.addMemoryAttr(llvm::MemoryEffects::none());
      FuncAttrs.addAttribute(llvm::Attribute::NoUnwind);
      // gcc specifies that 'const' functions have greater restrictions than
      // 'pure' functions, so they also cannot have infinite loops.
      FuncAttrs.addAttribute(llvm::Attribute::WillReturn);
    } else if (EnableIgnorePureConstAttrs && TargetDecl->hasAttr<PureAttr>()) {
    ...
    if (!getCodeGenOpts().DropNoAliasRestrictAttr && TargetDecl->hasAttr<RestrictAttr>())
  ...
  if (!IsThunk) {
    ...
    if (const auto *RefTy = RetTy->getAs<ReferenceType>()) {
      ...
      if (PTy->isObjectType()) {
        ...
        if (!(getCodeGenOpts().DropAlignAttr && (!Name.empty() && getCodeGenOpts().DropAlignAttrExcludeFunc.find(Name) == std::string::npos)))
  ...
  for (CGFunctionInfo::const_arg_iterator I = FI.arg_begin(),
    ...
    case ABIArgInfo::Indirect: {
      ...
      if (CodeGenOpts.PassByValueIsNoAlias && Decl &&
        ...
        Attrs.addAttribute(llvm::Attribute::NoAlias); // no need to guard with -fdrop-noalias-restrict-attr
      ...
      if (AI.getIndirectByVal() && (!(getCodeGenOpts().DropAlignAttr && (!Name.empty() && getCodeGenOpts().DropAlignAttrExcludeFunc.find(Name) == std::string::npos))))
...
void CodeGenFunction::EmitFunctionProlog(const CGFunctionInfo &FI,
  ...
  if (IRFunctionArgs.hasSRetArg()) {
    ...
    AI->addAttr(llvm::Attribute::NoAlias); // no need to guard with -fdrop-noalias-restrict-attr
  }
  ...
  for (FunctionArgList::const_iterator i = Args.begin(), e = Args.end();
    ...
    case ABIArgInfo::Direct: {
      ...
      if (ArgI.getDirectOffset() == 0 && LTy->isPointerTy() &&
        ...
        if (const ParmVarDecl *PVD = dyn_cast<ParmVarDecl>(Arg)) {
          ...
          if (const auto *ArrTy =
            ...
            if (ArrTy->getSizeModifier() == ArrayType::Static) {
              ...
              if (!(CGM.getCodeGenOpts().DropAlignAttr && (!Fn->getName().str().empty() && CGM.getCodeGenOpts().DropAlignAttrExcludeFunc.find(Fn->getName().str()) == std::string::npos)))
        ...
        if (!CGM.getCodeGenOpts().DropNoAliasRestrictAttr && Arg->getType().isRestrictQualified())
