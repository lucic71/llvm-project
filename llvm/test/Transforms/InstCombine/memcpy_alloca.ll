...
; RUN: opt < %s -passes=instcombine -zero-uninit-loads -S | FileCheck %s --check-prefixes=CHECK-ENABLED

; Memcpy is copying known-undef, and is thus removable
define void @test(ptr %dest) {
; CHECK-LABEL: @test(
; CHECK-NEXT:    ret void
; CHECK-ENABLED:    call void @llvm.memset.p0.i64(ptr noundef nonnull align 1 dereferenceable(7) %dest, i8 0, i64 7, i1 false) 
