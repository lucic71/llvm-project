...
; RUN: opt < %s -S -aa-pipeline=basic-aa -passes=inferattrs,dse | FileCheck %s --check-prefixes=CHECK
; RUN: opt < %s -S -aa-pipeline=basic-aa -passes=inferattrs,dse -disable-object-based-analysis | FileCheck %s --check-prefixes=CHECK-ENABLED
...
;
; CHECK-ENABLED-LABEL: @test_memcmp_const_size(
; CHECK-ENABLED-NEXT:  entry:
; CHECK-ENABLED-NEXT:    [[STACK:%.*]] = alloca [10 x i8], align 1
; CHECK-ENABLED-NEXT:    store i8 49, ptr [[STACK]], align 1
; CHECK-ENABLED-NEXT:    [[GEP_1:%.*]] = getelementptr i8, ptr [[STACK]], i64 1
; CHECK-ENABLED-NEXT:    store i8 50, ptr [[GEP_1]], align 1
; CHECK-ENABLED-NEXT:    [[GEP_2:%.*]] = getelementptr i8, ptr [[STACK]], i64 2
; CHECK-ENABLED-NEXT:    store i8 51, ptr [[GEP_2]], align 1
; CHECK-ENABLED-NEXT:    [[GEP_3:%.*]] = getelementptr i8, ptr [[STACK]], i64 3
; CHECK-ENABLED-NEXT:    store i8 52, ptr [[GEP_3]], align 1
; CHECK-ENABLED-NEXT:    [[RES:%.*]] = call i32 @memcmp(ptr nonnull dereferenceable(2) [[FOO:%.*]], ptr nonnull dereferenceable(2) [[STACK]], i64 2)
; CHECK-ENABLED-NEXT:    ret i32 [[RES]]
entry:
...
; CHECK-NEXT:    ret void
;
; CHECK-ENABLED-LABEL: @dse_strncpy_test4(
; CHECK-ENABLED-NEXT:    tail call void @llvm.memset.p0.i64(ptr [[OUT:%.*]], i8 42, i64 100, i1 false)
; CHECK-ENABLED-NEXT:    [[CALL:%.*]] = tail call ptr @strncpy(ptr [[OUT]], ptr [[IN:%.*]], i64 100)
...
; CHECK-NEXT:    ret void
;
; CHECK-ENABLED-LABEL: @dse_strncpy_test5(
; CHECK-ENABLED-NEXT:    tail call void @llvm.memset.p0.i64(ptr [[OUT:%.*]], i8 42, i64 100, i1 false)
; CHECK-ENABLED-NEXT:    [[CALL:%.*]] = tail call ptr @strncpy(ptr [[OUT]], ptr [[IN:%.*]], i64 99)
; CHECK-ENABLED-NEXT:    ret void
;
  tail call void @llvm.memset.p0.i64(ptr %out, i8 42, i64 100, i1 false)
