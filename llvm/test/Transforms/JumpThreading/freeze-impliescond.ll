...
; RUN: opt -passes=jump-threading -disable-undef-jump-threading -S < %s | FileCheck --check-prefixes=CHECK-DISABLE %s
...
;
; CHECK-DISABLE:       B:
; CHECK-DISABLE-NEXT:    br i1 [[DUMMYCOND:%.*]], label [[B2:%.*]], label [[DUMMY:%.*]]
; CHECK-DISABLE:       B2:
; CHECK-DISABLE-NEXT:    [[COND_FR2:%.*]] = freeze i1 [[COND:%.*]]
; CHECK-DISABLE-NEXT:    br i1 [[COND_FR2]], label [[UNREACHABLE:%.*]], label [[REACHABLE:%.*]]
