...
; RUN: opt < %s -passes=memcpyopt -S -verify-memoryssa -zero-uninit-loads | FileCheck %s --check-prefixes=CHECK-ZERO
...
;
; CHECK-ZERO-LABEL: @test1(
; CHECK-ZERO-NEXT:    [[BLETCH_SROA_1:%.*]] = alloca [7 x i8], align 1
; CHECK-ZERO-NEXT:    store i8 98, ptr [[FOOBIE:%.*]], align 4
; CHECK-ZERO-NEXT:    [[TMP1:%.*]] = getelementptr inbounds [[STRUCT_FOO:%.*]], ptr [[FOOBIE]], i64 0, i32 1, i64 0
; CHECK-ZERO-NEXT:    call void @llvm.memcpy.p0.p0.i64(ptr [[TMP1]], ptr [[BLETCH_SROA_1]], i64 7, i1 false) 
; CHECK-ZERO-NEXT:    [[TMP2:%.*]] = getelementptr inbounds [[STRUCT_FOO]], ptr [[FOOBIE]], i64 0, i32 2
; CHECK-ZERO-NEXT:    store i32 20, ptr [[TMP2]], align 4
; CHECK-ZERO-NEXT:    ret i32 undef
